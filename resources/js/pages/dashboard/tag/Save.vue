...
<template>
    <Head :title="tag.id ? 'Edit Tag' : 'Create Tag'" />
    ...
    <div class="mx-auto max-w-2xl space-y-6">
        ...
        <Heading
            :title="tag.id ? 'Edit Tag' : 'Create Tag'"
            description="Fill in the details below"
        />

        <Form
            v-bind="tag.id ? update.form(tag.id) : store.form()"
            class="space-y-0"
            v-slot="{ errors, processing, recentlySuccessful }"
        >
            <Card>
                <CardHeader class="space-y-1 pb-4">
                    <h3 class="text-lg font-medium">Tag Information</h3>
